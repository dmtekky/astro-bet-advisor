...
// Import the entire module and log its contents for debugging
const astroModule = require('./astroScore_2025-06-07.cjs');
console.log('Exported keys from astro module:', Object.keys(astroModule));
    ...
    for (let i = 0; i < players.length; i += batchSize) {
      ...
      for (const player of batch) {
        try {
          ...
          // Calculate astro score - try both direct function and module method
          let astroScore;
          if (typeof astroModule.calculateAstroInfluenceScore === 'function') {
            astroScore = await astroModule.calculateAstroInfluenceScore(
              {
          ...
          } else if (astroModule.default && typeof astroModule.default === 'function') {
            // Try default export if it exists
            astroScore = await astroModule.default(
              {
                player_id: player.player_id,
                player_birth_date: player.player_birth_date
              },
            ...
            throw new Error('No valid astro score calculation function found');
          }
