...
async function calculateAstroInfluenceScore(player, currentDate) {
  ...
  try {
    ...
    console.log(`Final score components for ${playerName}:`);
    ...
    console.log(`Final score for ${playerName}: ${boundedScore.toFixed(2)}`);
    console.log('---');
...
async function main(inputPlayersArray, inputCurrentDate) {
  if (!inputPlayersArray || inputPlayersArray.length === 0) {
    console.error('No players provided to main function.');
    return []; // Return empty if no players
  }
  console.log(`Processing ${inputPlayersArray.length} players provided as input.`);
  // Log the first player from input to verify structure
  if (inputPlayersArray.length > 0) {
    console.log('First input player sample:', JSON.stringify(inputPlayersArray[0], null, 2));
  }
  
  // Calculate scores for all players
  const scores = [];
  const currentDate = inputCurrentDate; // Use the passed current date
  
  for (const player of inputPlayersArray) {
    try {
  ...
  console.log('\nScore Statistics:');
  console.log(`- Players Processed: ${scores.length}`);
  ...
  const playerScoresData = inputPlayersArray.map((p, i) => {
    const score = scores[i];
    // Ensure player_id is consistently handled, similar to CSV generation
    const playerId = p.player_id || p.id || 'unknown_id'; 
    return {
      player_id: playerId,
      astro_influence_score: parseFloat(score.toFixed(2)) // Ensure score is a number
    };
  });
...
module.exports = {
  main,
  calculateAstroInfluenceScore // Also export this if it might be used directly elsewhere
};
