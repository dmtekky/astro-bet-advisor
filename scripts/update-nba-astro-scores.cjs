...
async function updatePlayerAstroScores(players) {
  ...
  for (let i = 0; i < players.length; i += BATCH_SIZE) {
    ...
    for (const player of batch) {
      try {
        // Prepare player data for astro score calculation
        const playerData = {
          ...player,
        ...
        // Add to updates if score changed
    ...
    // Update both nba_players and nba_player_season_stats_2025 tables
    for (const update of batchUpdates) {
      ...
      try {
        ...
        // Update nba_player_season_stats_2025 using the player's id as msf_player_id
        // since we saw that id matches external_player_id in the nba_players table
        ...
        try {
          ...
          // Find the stats record using msf_player_id (which should match the player's id)
          ...
          if (stats) {
            // Found the player - update using their ID
            ...
            lastError = new Error('No matching stats record found');
          }
        } catch (err) {
          lastError = err;
          ...
          if (!statsUpdated && lastError) {
            console.warn(`⚠️ Could not update stats for player ID ${player.id}:`, lastError.message);
            failed++;
          } else {
            updated++;
          }
        } else {
          console.warn(`⚠️ No external_player_id found for player ID ${player.id}`);
          // Don't count this as a failure since we still updated the main players table
          updated++;
        }
