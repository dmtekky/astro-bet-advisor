import { JulianDay, Time, Observer, House, Planet } from 'astronomia';
...
function calculateAspects(planetPositions) {
    ...
    for (let i = 0; i < planetPositions.length; i++) {
        for (let j = i + 1; j < planetPositions.length; j++) {
            ...
            if (diff > 180)
                diff = 360 - diff; // Get the shorter arc
...
export async function calculatePlanetaryPositions(birthData) {
    ...
    if (typeof birthData.timezoneOffset === 'number') {
        ...
        const localDate = new Date(Date.UTC(birthData.year, birthData.month - 1, birthData.day, birthData.hour, birthData.minute));
    ...
    const houseCusps = Object.values(houseData.cusp).slice(1);
